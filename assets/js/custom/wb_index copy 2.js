...
$("#publishBtnSales").on("click", function () {
	...
	populateSeoPublishForm();

	$("input[name='generateSeoOption'][value='no']")
		.prop("checked", true);

	$("#seoPublishForm").hide();

	$("#generateSeoPublishModal").modal("show");
});
function populateSeoPublishForm() {
	...
	const facebook = getCookie("clientFacebook") || "";
	const instagram = getCookie("clientInstagram") || "";
	const linkedin = getCookie("clientLinkedin") || "";
	const youtube = getCookie("clientYoutube") || "";

	$("#seoClientDomain").val("");

	$("#seoClientName").val(clientName);
	$("#seoProjectName").val(projectName);

	$("#seoBusinessName").val(clientName);

	$("#seoBusinessPhone").val(clientMobile);
	$("#seoBusinessEmail").val(clientEmail);

	$("#seoStreet").val(clientAddress);

	$("#seoFacebook").val(facebook);
	$("#seoInstagram").val(instagram);
	$("#seoLinkedin").val(linkedin);
	$("#seoYoutube").val(youtube);

	$("#seoBusinessType").val("");
	$("#seoBusinessDescription").val("");
	$("#seoPostalCode").val("");
	$("#seoCity").val("");
	$("#seoState").val("");
	$("#seoCountry").val("");
	...
	$("#seoProducts").val("");
	$("#seoAreasServed").val("");
	$("#seoBusinessHours").val("");
	$("#seoAdditionalInformation").val("");
}

$(document).on("change", "input[name='generateSeoOption']", function () {

	const value = $("input[name='generateSeoOption']:checked").val();

	if (value === "yes") {
		$("#seoPublishForm").stop(true, true).slideDown(200);
	} else {
		$("#seoPublishForm").stop(true, true).slideUp(200);
	}

});


function validateSeoDomainProjectName() {

	const domain = $("#seoClientDomain").val().trim();
	const projectName = $("#seoProjectName").val().trim();

	if (!domain) {
		showCustomAlertBox(
			"error",
			"Please enter your client domain."
		);
		return false;
	}
	...
	try {
		...
		showCustomAlertBox(
			"error",
			"Please enter a valid client domain."
		);

		return false;
	}

	const domainText = hostname
		.toLowerCase()
		.replace(/^www\./, "")
		.split(".")[0]
		.replace(/[^a-z0-9]/g, "");
	...
	if (
		!domainText ||
		!projectText ||
		!domainText.includes(projectText)
	) {

		showCustomAlertBox(
			"error",
			"You domain name and project name are not matching."
		);
...
const project_name =
	...
	fetch("/create_website/", {
		...
		body: new URLSearchParams({
			...
			generate_seo: seoData ? "true" : "false",
			client_domain: seoData ? seoData.clientDomain : "",
			seo_client_name: seoData ? seoData.clientName : "",
			seo_project_name: seoData ? seoData.projectName : "",
			client_details: seoData
				? JSON.stringify(seoData.businessData)
				: ""
		}),
	})
		// .then((res) => res.json())
...
async function updateSEO() {
    const clientDomain = "https://www.smilecare.com";
    const clientName = getCookie("clientName") || "";
    const projectName = getCookie("projectName") || "";
    ...
    const businessData = {
        ...
        type: "Dentist",
        description: "Dental clinic providing general, cosmetic and restorative dental care.",
        contact: {
        ...
        social_profiles: [
            ...
            "https://www.linkedin.com/company/smilecare-dental",
            "https://www.youtube.com/@smilecaredental"
        ],
        additional_information: "SmileCare Dental Clinic has been providing dental care in Pune since 2015. The clinic offers appointment-based services and provides treatment in English, Hindi and Marathi. Parking and wheelchair accessibility are available."
    };
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(data.error || "SEO optimization failed");
        }

        alert("SEO optimization completed successfully.");
...
$(document).on("click", "#updateSeoBtn", function () {
    updateSEO();
});


function getSeoPublishData() {

	const services = $("#seoServices")
		.val()
		.split(",")
		.map(item => item.trim())
		.filter(Boolean);

	const products = $("#seoProducts")
		.val()
		.split(",")
		.map(item => item.trim())
		.filter(Boolean);

	const areasServed = $("#seoAreasServed")
		.val()
		.split(",")
		.map(item => item.trim())
		.filter(Boolean);
	...
	const socialProfiles = [
		$("#seoFacebook").val().trim(),
		$("#seoInstagram").val().trim(),
		$("#seoLinkedin").val().trim(),
		$("#seoYoutube").val().trim()
	].filter(Boolean);

	const businessData = {

		name: $("#seoBusinessName").val().trim(),

		type: $("#seoBusinessType").val().trim(),

		description: $("#seoBusinessDescription")
			.val()
			.trim(),

		contact: {

			phone: $("#seoBusinessPhone")
				.val()
				.trim(),

			email: $("#seoBusinessEmail")
				.val()
				.trim()
...
$("#publishSeoServerBtn").on("click", function () {

	const generateSeo =
		$("input[name='generateSeoOption']:checked").val();
	...
	if (!validateSeoDomainProjectName()) {
		return;
	}

	const seoData = getSeoPublishData();
