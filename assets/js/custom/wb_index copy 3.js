...
$(document).on("input", "#seoClientDomain", function () {
    ...
    const isValid = projectParts.some(part => {
        if (part.length <= 3) {
            return domainText.includes(part);
        }

        for (let i = 0; i <= part.length - 3; i++) {
            if (domainText.includes(part.substring(i, i + 3))) {
                return true;
            }
        }
    ...
    if (!isValid) {
        const originalProjectName = getCookie("projectName") || "";

        errorBox
            .removeClass("valid")
            .text("Domain should include at least 3 letters from the project name: " + originalProjectName)
            .show();
